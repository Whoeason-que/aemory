...
def built_large_dataset(benchmark, large_md_dataset):
    ...
    benchmark(lambda: aemory.build(large_md_dataset, output_path, "BAAI/bge-small-en-v1.5"))
    yield output_path
    if os.path.exists(output_path):
        shutil.rmtree(output_path)
...
class TestBuildBenchmark:
    def test_benchmark_build_small(self, benchmark, tmp_path):
        ...
        benchmark(lambda: aemory.build(str(md_path), output, "BAAI/bge-small-en-v1.5"))
...
class TestSearchBenchmark:
    def test_benchmark_search_single(self, benchmark, built_large_dataset):
        benchmark(lambda: aemory.search(built_large_dataset, "benchmark content", 5, "BAAI/bge-small-en-v1.5"))
    ...
    def test_benchmark_search_multiple_queries(self, benchmark, built_large_dataset):
        ...
        def search_all():
            return [aemory.search(built_large_dataset, q, 3, "BAAI/bge-small-en-v1.5") for q in queries]
        
        benchmark(search_all)

    def test_benchmark_search_large_limit(self, benchmark, built_large_dataset):
        benchmark(lambda: aemory.search(built_large_dataset, "content", 50, "BAAI/bge-small-en-v1.5"))
...
class TestLatencyBenchmark:
    def test_search_latency_p50(self, built_large_dataset):
        ...
        for _ in range(100):
            ...
            aemory.search(built_large_dataset, query, 10, "BAAI/bge-small-en-v1.5")
    ...
    def test_throughput(self, built_large_dataset):
        ...
        while time.perf_counter() - start < duration:
            aemory.search(built_large_dataset, query, 5, "BAAI/bge-small-en-v1.5")
            count += 1
